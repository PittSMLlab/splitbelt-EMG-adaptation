...
dAllMarkers=expData.data{trial}.markerData.lowPassFilter(10).derivative.split(t1,t2).substituteNaNs;
...
switch dataSets
    case 1 %midHip
        ...
        Data=(RHIP+LHIP)*.5;
        dData=Data.lowPassFilter(10).derivative;
        clear RHIP LHIP
    case 2 %COP
        [COP,~,~]=expData.data{trial}.computeCOPAlt;
        Data=COP.split(t1,t2).getDataAsOTS('COP').resample(Ts,T0);
        dData=COP.getDataAsOTS('COP').lowPassFilter(10).derivative.split(t1,t2).resample(Ts,T0);
    case 3 %COP mix
        %[~,COPL,COPR]=expData.data{trial}.computeCOPAlt;
        [COP,~,~]=expData.data{trial}.computeCOPAlt;
        COP1=COP.medianFilter(5).substituteNaNs.lowPassFilter(30).split(t1,t2).resample(Ts,T0);
        COPL=COP1.getDataAsOTS('LCOP');
        COPR=COP1.getDataAsOTS('RCOP');
        Data=(COPR.* (stepIdx==1)) + (COPL .* (stepIdx==-1));
        dCOP1=COP.medianFilter(5).substituteNaNs.lowPassFilter(10).derivative.split(t1,t2).resample(Ts,T0);
        dCOPL=dCOP1.getDataAsOTS('d/dt LCOP');
        dCOPR=dCOP1.getDataAsOTS('d/dt RCOP');
