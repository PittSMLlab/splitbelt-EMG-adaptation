...
%% Open fig
oldFig=openfig(['../../fig/all/emg/' name]);
if exist('name2','var')
    oldFig2=openfig(['../../fig/all/emg/' name2]);
else
    oldFig2=oldFig;
end
...
p2=plot(xOff/12-1.4*[1 1]/12,[17 31]-.5,'Color',ccc(2,:),'LineWidth',6,'Clipping','off');
t2=text(xOff/12-2/12,19,'SLOW/PARETIC','FontSize',16,'Rotation',90,'Color',1*ccc(2,:));
yOff=0;
...
if length(yCoord)<17
   ...
   delete(t2)
    ...
    newAxes.YTickLabel=cellfun(@(x) x([2,max(3,length(x)-1):length(x)]),newAxes.YTickLabel,'UniformOutput',false);
    maxI=2;
end
...
if dualPlot
    ...
    titles=get(plots,'Title');
    strTitle=cellfun(@(x) get(x,'String'),titles,'UniformOutput',false);
    descriptionMatch2=find(all(cell2mat(cellfun(@(x) contains(strTitle,x,'IgnoreCase',true),desiredPlotDescription2,'UniformOutput',false)),2));
