...
for i=1:size(allTS,2)
    for sIdx=1:size(allTS,1)
        ...
        % Get derivatives
        thisTS=thisTS.cat(thisTS.lowPassFilter(10).derivative);
        thisEMG=thisEMG.cat(thisEMG.substituteNaNs.lowPassFilter(10).derivative).split(thisEMG.Time(1),thisTS.Time(end)+1e-5);
        ...
        if sIdx==1 %First sub
            ...
            %medianTS{i}=medianTS{i}.cat(alignTS.getPartialDataAsATS(labelOrder).mean);
        end
    end
end
