...
for dataSet=1%:2%3
    switch dataSet
        case 1 %Using adaptation data to fit model
            Y=[summaryFunction(dataSym{1}); summaryFunction(dataSym{2})];
...
%             nullBD=false;
    ...
    for Nfolds=1:2 %Doing 1,2-fold cross-validation 
        for order=2:3%1:2
            model(end+[1:Nfolds]) = CV_EM(Y',U',order,Nfolds,'fast');
...
model{end+1}.J=zeros(0,0); model{end}.C=zeros(size(Yall',1),0); model{end}.D=zeros(size(Yall',1),1);
model{end}.X=zeros(0,size(dataSym{1},1)); model{end}.name='Baseline [0]'; model{end}.B=zeros(0,1);
model{end}.Q=[]; model{end}.R=[]; model{end}.Xmle=zeros(0,size(Yall',2)); model{end}.Xsim=zeros(0,size(Yall',2)); model{end}.Xproj=zeros(0,size(Yall',2)); 
model{end}.Ysim=zeros(size(Yall'));  %model{end}.Ymle=zeros(size(Yall'));
...
for k=1:length(model)-1 %Excepting baseline model
    model{k}.Xproj=model{k}.C\(Yall'-model{k}.D*Uall'); %Projecting data onto C's span
    [model{k}.Ysim,model{k}.Xsim]=fwdSim(Uall',model{k}.J,model{k}.B,model{k}.C,model{k}.D,zeros(size(model{k}.B)),[],[]); %Smooth simulation
    model{k}.Xmle=statKalmanSmoother(Yall',model{k}.J,model{k}.C,model{k}.Q,model{k}.R,[],[],model{k}.B,model{k}.D,Uall');
end
    
for k=1:length(model)
    model{k}.Ymle=model{k}.C * model{k}.Xmle + model{k}.D * Uall';  %Simulation of output with MLE states
    model{k}.resSmooth=Yall'-model{k}.Ysim;
    model{k}.resBest=Yall'-model{k}.Ymle;
    model{k}.res=model{k}.resSmooth;
...
if saveFlag
    aux=func2str(summaryFunction);
    save(['../data/dynamicsModelingResultsALL_' aux(5:end-5) 'EM.mat'], 'model', 'Yall', 'Uall', 'model', 'dataSym', 'summaryFunction');
end
%% Plot basic model performance
%If the previous cell was not run, load:
summaryFunction=@(x) median(x,3); %Using mean or median
aux=func2str(summaryFunction);
load(['../data/dynamicsModelingResultsALL_' aux(5:end-5) 'EM.mat'])
...
if saveFlag
    aux=func2str(summaryFunction);
    saveFig(fh,'../intfig/all/dyn/',['EMmodelOrderAssessment_' aux(5:end-5)],0)
end
...
aux='bilateral';
load(['../data/dynamicsModelingResultsALL_' aux 'EM.mat'])
...
for i=2%[1,2,3,4,5]%15,16]
    ...
    if saveFlag
        saveFig(fh,'../intfig/all/dyn/',['EM' regexprep(model{i}.name,'/','_') aux(5:end-5) 'ALT']);
    end
end
